...
const CommoditiesPage = () => {
  ...
  // Placeholder images to prevent Vite from crashing
  ...
  const spicesImg = "https://images.unsplash.com/photo-1596040033229-a9821ebd058d?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80";

  const filters = ["Coffee", "Cocoa", "Spices", "Coconut", "Rubber", "Palm"];

  // 2. THE BACKEND CONNECTION
  useEffect(() => {
    // This function will eventually pull data from your Express backend
    const fetchCommodities = async () => {
      try {
        // LATER: Replace the mockData below with this real API call:
        // const response = await fetch('http://localhost:5000/api/commodities');
        // const data = await response.json();
        // setCommodities(data);

        // FOR NOW: Loading the exact same dummy data so the UI works today.
        const mockData = [
          { img: coffeeImg, name: "Gayo Arabica", origin: "Aceh", grade: "Grade 1 · Washed", tags: ["Coffee"] },
          { img: cocoaImg, name: "Sulawesi Cocoa", origin: "Central Sulawesi", grade: "Fermented · 100g", tags: ["Cocoa"] },
          { img: spicesImg, name: "Banda Nutmeg", origin: "Maluku", grade: "ABCD · Sortex", tags: ["Spices"] },
          { img: coffeeImg, name: "Toraja Arabica", origin: "Sulawesi", grade: "Grade 1 · Semi-washed", tags: ["Coffee"] },
          { img: spicesImg, name: "Java Clove", origin: "East Java", grade: "Hand-picked", tags: ["Spices"] },
          { img: cocoaImg, name: "Lampung Cocoa", origin: "Lampung", grade: "Sun-dried", tags: ["Cocoa"] },
        ];

        // Simulating network delay just so it feels real
        setTimeout(() => {
  ...
  const visible = active.length 
    ? commodities.filter((i) => i.tags.some((t) => active.includes(t))) 
    : commodities;
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-16">
      ...
      <div className="grid md:grid-cols-[240px_1fr] gap-10">
        ...
        {loading ? (
          <div className="grid place-items-center h-64 text-gray-500">Loading commodities...</div>
        ) : (
            ...
            {visible.map((it, i) => view === "grid" ? (
              ...
              <article key={i} className="bg-white rounded-lg overflow-hidden shadow-md group border border-gray-100">
                ...
                <div className="p-5">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">{it.origin} · {it.grade}</p>
              ...
              <article key={i} className="flex flex-col sm:flex-row gap-5 bg-white rounded-lg p-4 shadow-sm border border-gray-100 items-start sm:items-center">
                <img src={it.img} alt={it.name} loading="lazy" className="h-24 w-24 object-cover rounded shrink-0" />
